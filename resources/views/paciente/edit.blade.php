...
@section('conteudo')
    ...
    <form action="{{route('paciente.update', $paciente->id)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="input-group mb-3">
                    ...
                    <select name="dieta" class="form-select form-control">
                        @foreach ($dietas as $item)
                            <option
                                value="{{$item->id}}"
                                @if($item->id == $paciente->dieta_id) selected="true" @endif
                            >
                                {{ $item->nome }}
                            </option>
                        @endforeach
        ...
        <div class="row mb-5">
            <div class="col">
                <a href="{{route('paciente.index')}}" class="btn btn-secondary btn-block align-content-center">
